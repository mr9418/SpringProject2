	...
	<table>
		...
		<c:if test="${pagingVO.totalCount>0 }">
			<c:forEach var="vo" items="${pagingVO.list}" varStatus="vs">
				<tr>
					<td>${vo.idx }</td>
					<td style="text-align: left"><span id="content${vo.idx }"><c:out
								value="${vo.content }" /></span></td>
					<td><span id="name${vo.idx }"><c:out
			...
			<tr>
				<td colspan="5" style="border: none; text-align: left">
					<form action="guestinsertOk" method="post" id="memoForm">
						<input type="hidden" name="idx" value="0" id="idx"> <input
							type="hidden" name="p" value="${commonVO.currentPage }" id="idx">
						...
						<input type="text" name="name" id="name" style="width: 70px;"
							...
							style="width: 70px;" placeholder="암호" required="required"><br>
						<input type="text" name="content" id="content" placeholder="내용"
							required="required" size="73"> <br /> <input
							type="submit" value="저장" class="btn btn-outline-dark btn-sm"
							id="submitBtn"> <input type="button" value="돌아가기"
		...
		<c:if test="${vo == null }">
			<table>
					...
					<td id="tt">로그인하셔야 작성이 가능합니다 &nbsp;&nbsp;
						<button class="btn btn-success btn-sm"
							onclick='post_to_url("${pageContext.request.contextPath }/",{"p":${commonVO.currentPage },"s":${commonVO.pageSize},"b":${commonVO.blockSize }})'>홈으로
							돌아가기</button>
					</td>
